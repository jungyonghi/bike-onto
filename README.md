...
- `inspect-dir`: 파편화된 domain artifact directory를 검사하고 실행 대상 파일을 고정
- zero-arg `./bike`: first-run setup 후 바로 chat loop 진입
- Pi agent extension: `/bike-setup`, `/bike-status`, `/bike-tools`와 LLM-callable RAG inspection tools 등록
...
- `wiki-export`: 평가 결과를 Obsidian ontology-like wiki로 export
- `benchmark-polish`: QA benchmark를 실사용 문체와 answerability/review policy에 맞게 정리
- `ontology-map`: NODEPROMPT-inspired radial/tree ontology visual 생성
- `pgvector-*`: live PostgreSQL/pgvector retrieval adapter
...
| Tests | `187 passed, 3 warnings` on packaged fixtures and smoke/regression tests |
...
OBYBK는 RAG를 “답변 생성기”가 아니라 **검증 가능한 지식 산출 workflow**로 다룹니다.
...
| Review Gate | 자동 수용 여부, 사람 검토 사유, data gap |
| Answerability | 실행 가능성 또는 제한 사유의 분류 |
| Natural Label | `gender=1.0` 같은 내부 코드를 `남성(M)`처럼 사용자-facing label로 변환 |

---

## Scope Note: Ontology-Hybrid

OBYBK의 ontology 설계는 Alexander Osterwalder의 **Business Model Ontology**를 OWL 2 DL 스타일로 형식화한 내부 청사진을 기반으로 합니다.
...
즉, OBYBK의 ontology는 완전한 논리 추론 엔진이 아니라 **RAG 답변을 inspectable하게 만드는 의미 구조화 계층**입니다.

---

## Ontology Seed Anchor: Business Model Ontology

OBYBK의 ontology seed는 임의의 태그 묶음이 아니라, Alexander Osterwalder의 **Business Model Ontology** 논문을 anchor로 삼아 구현했습니다.

OBYBK의 큰 축은 LLM이 즉흥적으로 만든 분류가 아니라, 논문 기반 개념 모델에서 출발합니다. LLM은 표현 정리와 후보 확장을 보조하지만, 최상위 ontology axis는 Business Model Ontology의 pillar/element와 OWL 2 DL-style blueprint에 의해 고정됩니다.

핵심 흐름: **Business Model Ontology paper → OWL 2 DL-style blueprint → upper ontology seed → domain benchmark → inspectable RAG graph**

```mermaid
flowchart LR
    BMO[Business Model Ontology paper] --> BP[OWL 2 DL-style blueprint]
...
| Layer | Role |
|---|---|
| Formalization Blueprint | BMO pillar/element를 OWL 2 DL-style 구조로 고정 |
| Upper Ontology Seed | 재사용 가능한 class/relation 후보를 생성 |
| Runtime Projection | RAG 답변을 claim/evidence/entity/relation/review gate로 투영 |
...
논문의 4개 pillar와 9개 business model element를 OWL 2 DL 스타일로 정리한 청사진을 먼저 만들었습니다.
...
청사진을 바로 runtime OWL reasoner로 실행하지 않고, RAG inspection에 재사용 가능한 upper ontology seed로 변환했습니다.

```text
tools/scripts/generate_codex_upper_ontology_seed.py
```

이 generator는 1,000개 규모의 `candidate_record`와 `promotion_record`를 생성합니다.
...
Seed category는 Business Model Ontology의 핵심 축을 일반화해 구성했습니다.
...
즉, seed는 단순 명사 목록이 아니라 RAG 답변을 검사하는 데 필요한 **정체성, 경계, 역할, 상태, 관계, 생애주기, 근거** 축을 함께 생성합니다.
...
현재 runtime은 full OWL reasoner가 아닙니다. 대신 Business Model Ontology anchor와 upper seed를 RAG inspection에 맞게 투영합니다.
...
이 구조가 `VisualGraphPayload`, Visual Inspector, Evaluation Overview, Obsidian Wiki Export의 공통 골격입니다. QP/QO benchmark는 이 seed를 실제 domain artifact에 맞게 보완하는 competency-question layer로 사용됩니다.

정리하면:

```text
Business Model Ontology = conceptual seed
...
OBYBK는 “지금 숫자를 못 냄”과 “사람 검토가 필요함”을 분리합니다.
...
이 정책은 benchmark 문서뿐 아니라 RAG answer payload에도 들어갑니다.
...
사용자-facing 답변에서는 내부 코드값보다 자연어 label을 우선합니다.
...
raw 값은 보존하고 display label을 추가해, **검증 가능성**과 **사용자 가독성**을 동시에 유지합니다.
...
CLI 답변에서 `--visual-click`을 켜면 답변의 entity mention과 후보 entity가 clickable link로 출력됩니다. Ctrl+Click 또는 `--open-visual-app`으로 local visual page를 열면, 답변별 ontology tree/radial graph와 entity card를 함께 볼 수 있습니다.
...
같은 visual page에서 `Ontology Tree` / `Radial Map` 버튼으로 구조를 전환할 수 있습니다.
...
OBYBK는 claim/evidence/entity/relation/review gate가 어떻게 연결되는지 한 장에서 확인할 수 있는 ontology-like visual map을 제공합니다.
...
단일 답변은 Visual Inspector로, 여러 평가 결과는 Evaluation Overview로 확인합니다.
...
평가 결과는 Obsidian vault로 export됩니다. 단순 Markdown dump가 아니라 frontmatter, tag, backlink를 포함한 ontology-like knowledge projection입니다.
...
OBYBK의 100문항 benchmark는 원칙적으로 **도메인별 생성형**입니다. 고정된 질문 100개가 모든 도메인의 정답셋이라는 뜻이 아닙니다. 새 domain artifact를 넣으면 LLM이 해당 도메인의 schema, source, entity, relation 후보를 읽고 QP/QO competency questions를 생성한 뒤, deterministic answerability/review policy로 검증하고 versioned benchmark snapshot으로 고정하는 구조가 맞습니다.
...
현재 100문항은 Seoul Bike case-study binding으로 만든 **reference snapshot**이며, universal fixed benchmark가 아닙니다.
...
실제 parquet를 실행해 숫자와 대상까지 채운 user-facing 예시도 포함했습니다.
...
이 결과는 대규모 통계 benchmark가 아니라, **어떤 질문 유형에서 inspection layer가 유용한지 보여주는 diagnostic snapshot**입니다.
...
TTY에서 `./bike`를 처음 실행하면 setup wizard를 먼저 열고, setup 이후에는 바로 채팅 루프로 들어갑니다.
...
Project-local extension은 `.pi/extensions/bike-onto/index.ts`에 있으며, repo root에서 `pi`를 실행하면 auto-discovery 대상입니다.
...
extension auto-discovery가 꺼져 있거나 빠른 테스트만 하려면:

```bash
pi -e .pi/extensions/bike-onto/index.ts
```

extension 파일을 수정한 뒤에는 Pi 안에서:
...
|---|---|---|
| `bike_rag_answer` | 질문에 대한 grounded RAG answer + inspection payload 반환 | `./bike ask --json` |
...
| `bike_wiki_export` | evaluation results를 Obsidian ontology-like wiki로 export | `./bike wiki-export` |
...
이 평가 결과를 Obsidian wiki로 export해줘.
```
...
- `status`/`/bike-status`는 secret 값을 출력하지 않습니다.
